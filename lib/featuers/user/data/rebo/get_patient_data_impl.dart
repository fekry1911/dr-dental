...
class GetPatientDataImpl extends GetPatientData{
  ...
  Future<List<BookingModel>> getPatientAppointments(String id) async {
    ...
    var bookings=await firebaseFirestore.collection("patients").doc(id).collection("bookings").orderBy(FieldPath.documentId, descending: true).get();
    bookingsModels=bookings.docs.map((e) => BookingModel.fromMap(e.data(),e.id)).toList();
  ...
  Future deletePatient(String id) async {
   ...
   var bookings= await firebaseFirestore.collection("patients").doc(id).collection("bookings").get();
   bookingIds=bookings.docs.map((doc) => doc.id).toList();
   bookingIds.forEach((element) async {
     await firebaseFirestore.collection("booking").doc(element).collection("bookings").doc(id).delete();
   });
  ...
  Future deletePatientAppointments(String patientId,String bookID) async {
    await firebaseFirestore.collection("patients").doc(patientId).collection("bookings").doc(bookID).delete();
    await firebaseFirestore.collection("booking").doc(bookID).collection("bookings").doc(patientId).delete();
  }
}